    ...
    <div className={`${quicksand.variable} ${nunitoSans.variable} min-h-screen bg-[#F0F9FF] text-gray-900`}>
      ...
      <section className="relative min-h-screen overflow-hidden bg-gradient-to-b from-[#E0F2FE] via-[#F0F9FF] to-white">
        ...
        {heroImg && (
          <div className="absolute inset-0">
            <img src={heroImg} alt={`${props.firmName} cam balkon`} className="h-full w-full object-cover" style={{ opacity: 0.12 }} />
            <div className="absolute inset-0 bg-gradient-to-r from-[#F0F9FF]/95 via-[#F0F9FF]/85 to-[#F0F9FF]/40" />
          </div>
        )}
        ...
        <div className="relative mx-auto flex min-h-screen max-w-7xl items-center px-4 sm:px-6 lg:px-8">
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="max-w-2xl">
            ...
            <motion.p variants={fadeInUp} className="mt-5 max-w-lg font-[family-name:var(--font-nunito-sans)] text-lg leading-relaxed text-gray-500">
              {props.firmName} — {props.city}&apos;de katlanir cam balkon, surme cam balkon, isicamli sistem. Balkonunuz dort mevsim yasam alani.
            </motion.p>
            <motion.div variants={fadeInUp} className="mt-6 flex flex-wrap gap-3">
              {['Katlanir', 'Surme', 'Isicamli', 'Menteseli', 'Seri Sistem'].map((s) => (
      ...
      <section className="bg-[#E0F2FE] py-20"><div className="mx-auto max-w-4xl px-4 sm:px-6">
        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center">
          ...
          <motion.h2 variants={fadeInUp} className="font-[family-name:var(--font-quicksand)] text-3xl font-700 text-gray-900 sm:text-4xl">Cam balkon arayanlar <span className="text-red-500">gorsel ve referans</span> karsilastiriyor</motion.h2>
          <motion.p variants={fadeInUp} className="mt-4 font-[family-name:var(--font-nunito-sans)] text-lg text-gray-500">Web siteniz yoksa musteriler sizi atlayip rakibinizden teklif aliyor.</motion.p>
        </motion.div>
      </div></section>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <motion.div variants={slideFromLeft} className="rounded-2xl border border-[#0EA5E9]/20 bg-[#E0F2FE] p-8"><div className="mb-4 text-2xl">✅</div><h3 className="mb-4 font-[family-name:var(--font-quicksand)] text-lg font-600 text-gray-900">Olursa</h3>
              <ul className="space-y-3 font-[family-name:var(--font-nunito-sans)] text-sm text-gray-600">
                {['Proje gorselleri ile guven olusturursunuz', 'Sistem secenekleriniz seffaf sunulur', 'Online teklif formu musteri kazandirir', 'Google\'da "cam balkon + sehir" aramasinda cikabilirsiniz', 'Garanti kosullariniz gorunur olur'].map((i) => (
            ...
            <motion.div variants={slideFromRight} className="rounded-2xl border border-red-200 bg-red-50 p-8"><div className="mb-4 text-2xl">❌</div><h3 className="mb-4 font-[family-name:var(--font-quicksand)] text-lg font-600 text-gray-900">Olmazsa</h3>
              <ul className="space-y-3 font-[family-name:var(--font-nunito-sans)] text-sm text-gray-600">
                {['Musteriler bilinen markalara yonelir', 'Referanslariniz bilinmez', 'Fiyat teklifi almak zorlasiyor', 'Kalite farkiniz gorulmuyor', 'Profesyonel imaj olusturulamiyor'].map((i) => (
      ...
      <section className="bg-[#0EA5E9] py-20"><div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <motion.h2 variants={fadeInUp} className="font-[family-name:var(--font-quicksand)] text-3xl font-700 text-white sm:text-4xl">{props.firmName} Icin<br />Profesyonel Web Sitenizi Olusturalim</motion.h2>
          <motion.p variants={fadeInUp} className="mt-4 font-[family-name:var(--font-nunito-sans)] text-white/80">Ucretsiz demo — taahhut yok.</motion.p>
          <motion.div variants={fadeInUp} className="mt-8"><a href={chatLink} onClick={() => trackEvent(props.slug, 'CHAT_CLICK')} className="inline-flex items-center gap-2 rounded-xl bg-white px-10 py-4 font-[family-name:var(--font-quicksand)] text-sm font-700 text-[#0EA5E9] shadow-lg transition-all hover:bg-gray-50">Ucretsiz Teklif Al <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg></a></motion.div>
        </motion.div>
      </div></section>

      <section className="py-16"><div className="mx-auto max-w-3xl px-4 text-center sm:px-6"><motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}><motion.div variants={scaleIn} className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#0EA5E9]/10"><span className="text-2xl">🔒</span></motion.div><motion.h3 variants={fadeInUp} className="font-[family-name:var(--font-quicksand)] text-lg font-700 text-gray-900">Taahhut Yok, Risk Yok</motion.h3><motion.p variants={fadeInUp} className="mt-2 font-[family-name:var(--font-nunito-sans)] text-sm text-gray-400">Demo tamamen ucretsizdir.</motion.p></motion.div></div></section>

      <footer className="border-t border-gray-100 py-8"><div className="mx-auto max-w-5xl px-4 text-center sm:px-6"><p className="font-[family-name:var(--font-nunito-sans)] text-xs text-gray-300">© {new Date().getFullYear()} Vorte Studio — {props.firmName} icin ozel olarak hazirlanmistir.</p></div></footer>
    </div>
  )
}
