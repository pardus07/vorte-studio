  ...
  return (
    <div className={`${nunito.variable} ${inter.variable} min-h-screen bg-[#FFFDF7] text-[#1C1917]`}>
      ...
      >
        ...
        <div className={`relative mx-auto ${props.images?.hero ? 'max-w-6xl grid grid-cols-1 lg:grid-cols-2 items-center gap-12' : 'max-w-3xl text-center'}`}>
          {/* Hero Text Column */}
          <div>
            ...
            <motion.p
              variants={fadeInUp}
              className={`mt-5 font-[family-name:var(--font-inter)] text-base text-[#1C1917]/70 md:text-lg ${props.images?.hero ? 'max-w-lg' : 'mx-auto max-w-xl'}`}
            >
            ...
            {/* CTA Buttons */}
            <motion.div variants={fadeInUp} className={`mt-8 flex flex-col gap-4 sm:flex-row ${props.images?.hero ? '' : 'items-center sm:justify-center'}`}>
              <a
                href={chatLink}
          ...
          {props.images?.hero && (
            ...
            >
              <div className="h-80 w-80 overflow-hidden rounded-3xl shadow-2xl shadow-[#10B981]/20">
                <img
