...
export default function PortalChat({ initialMessages }: { initialMessages: Message[] }) {
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-7rem)] max-w-3xl flex-col lg:h-[calc(100vh-4rem)]">
      ...
      {/* Mesaj alanı */}
      <div className="flex-1 overflow-y-auto rounded-2xl border border-white/[0.07] bg-[#0c0c0e] p-4">
        {messages.length === 0 ? (
              ...
              return (
                <div key={m.id}>
                  ...
                  <div className={`flex ${isCustomer ? "justify-end" : "justify-start"}`}>
                    ...
                    >
                      {!isCustomer && (
                        <p className="mb-1 text-[10px] font-semibold text-[#FF4500]/70">Vorte Studio</p>
                      )}
      ...
      <div className="mt-3 flex gap-2">
        <textarea
          ...
          rows={1}
          className="flex-1 resize-none rounded-xl border border-white/[0.07] bg-[#0f0f0f] px-4 py-3 text-sm text-white placeholder-white/25 outline-none transition-colors focus:border-[#FF4500]/30"
        />
        <button
          onClick={handleSend}
          disabled={!input.trim() || sending}
          className="flex h-[46px] w-[46px] flex-shrink-0 items-center justify-center rounded-xl bg-[#FF4500] text-white transition-opacity disabled:opacity-30"
        >
